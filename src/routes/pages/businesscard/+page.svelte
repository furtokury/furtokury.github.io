<script>
  ...
  onMount(() => {
    ...
    fetch(`/businesscard/2.json`)
      .then((response) => response.json())
      .then((data) => {
        businessCard2Data = data;
      });
  });
</script>
    ...
    <div class="business-card-section">
      <FadeInAnimation>
        ...
        <div class="image-container">
          <img src="/images/business-card-2.png" alt="Business Card #2" class="business-card-image" />
          <img src="/images/business-card-2-back.svg" alt="Business Card Back #2" class="business-card-image" />
        </div>
        <div class="image-container-description">
          {$_("businesscards.illust2.description")}
        </div>
      </FadeInAnimation>
    </div>
    <div class="owners-section">
      <FadeInAnimation>
        ...
        <div class="owners-list">
          {#each businessCard2Data.owners as _, i}
            <div class="owner-entry">
              #{businessCard2Data.owners[i].id} <strong>{businessCard2Data.owners[i].name}</strong>
            </div>
          {/each}
        </div>
      </FadeInAnimation>
